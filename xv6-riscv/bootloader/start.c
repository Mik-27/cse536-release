...
void start()
{
  ...
  /* CSE 536: With kernelpmp2, isolate 118-120 MB and 122-126 MB using NAPOT */ 
    ...
    w_pmpaddr1(((KERNBASE + 120*1024*1024) >> 2) + ((2*1024*1024) >> 3));  // Accessible

    // w_pmpaddr3(((KERNBASE + 122*1024*1024) >> 2) + ((4*1024*1024) >> 3));  // Inaccessible

    w_pmpaddr2(((KERNBASE + 126*1024*1024) >> 2) + ((2*1024*1024) >> 3));   // Accessible
    
    w_pmpcfg0(0x1f1f0f); // pmp3cfg + pmp2cfg + pmp1cfg + pmp0cfg
    // w_pmpcfg1(0x1f);
  #endif
